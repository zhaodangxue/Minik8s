...
func PrepareBuildEnv(input *apiobjects.Job) (buildPath string, err error) {
	...
	// 拷贝用户文件
	err = utils.CopyDir(input.Spec.BuildOptions.FunctionFileDir, buildPath+input.Spec.BuildOptions.FunctionFileDir)
	if err != nil {
	...
	// 加入用户自定义的命令
	dstFile.WriteString("\n")
	for _, command := range input.Spec.BuildOptions.ExtraCommands {
		dstFile.WriteString("\n")
	...
	// 加入默认文件
	dstFile.WriteString("\n")
	copyDir := "COPY " + input.Spec.BuildOptions.FunctionFileDir + " /function"
